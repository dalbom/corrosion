#!/bin/bash
# GPU 0: DDPM — 15 sensors × 5 trials (skips missing checkpoints)
set -e
...
echo "=========================================="
echo "GPU 0: DDPM evaluation pipeline"
echo "=========================================="

for trial in 1 2 3 4 5; do
    seed=${SEEDS[$((trial-1))]}
    echo ""
    echo "--- DDPM Trial $trial (seed=$seed) ---"

    for sensor in "${ALL_SENSORS[@]}"; do
        channels=$(echo "$sensor" | tr '_' ' ')
        ckpt=$(find "checkpoints/baseline/ddpm/$sensor" -name "model_step_1000000.pt" -type f 2>/dev/null | head -1)
        outdir="results/baseline/ddpm/trial${trial}/${sensor}"
        corrdir="results/corrected/ddpm/trial${trial}/${sensor}"
...
import sys; sys.argv = ['inference.py',
    '--checkpoint', '$ckpt',
    '--csv', '$TEST_CSV',
    '--output', '$outdir',
    '--img_root', '$IMG_ROOT',
...
"
    ...
    out_csv="output/notion/DDPM_${trial}.csv"
    mkdir -p output/notion
    echo "Index,Sensor Type,MAE,MSE,PSNR,SSIM,MACE,비고" > "$out_csv"

    for sensor_dir in results/corrected/ddpm/trial${trial}/*/; do
        ...
        result=$(python evaluate_metrics.py \
            ...
            --gen_root "results/corrected/ddpm/trial${trial}/$sensor" \
...
echo ""
echo "GPU 0: DDPM pipeline complete!"
